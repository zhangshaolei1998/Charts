...
FIG_NAME = "probe_slices_dotplot_v1"
...
def band(ax, y: float) -> None:
    """Alternating row stripe: x spans the axes, y is in data coordinates."""
    ax.add_patch(
        Rectangle(
            (0, y - 0.5),
            1.0,
...
def draw_dot_panel(ax, rows: list[Row], title: str, show_xlabel: bool) -> None:
    ...
    if show_xlabel:
        ax.set_xlabel("Judge score (task mean)", fontsize=7.6, labelpad=2)
...
def draw_delta_panel(ax, rows: list[Row], show_xlabel: bool, annotate: bool = False) -> None:
    ...
    ax.tick_params(axis="x", length=2.5, width=0.6)
    if show_xlabel:
        ax.set_xlabel(r"$\Delta$ Chart$-$Text", fontsize=7.6, labelpad=2)
    if annotate:
        ax.text(
            0.98,
            1.012,
...
def main() -> None:
    set_style()
    q_rows = by_delta(QUESTION_TYPE) + [ALL_TASKS]
    c_rows = by_delta(COGNITIVE)
    n_top, n_bot = len(q_rows), len(c_rows)

    fig = plt.figure(figsize=(TEXT_WIDTH_IN, 4.85))
    gs = fig.add_gridspec(
        2,
        2,
    ...
    draw_dot_panel(ax_q, q_rows, "(a) By question type", show_xlabel=False)
    draw_delta_panel(ax_qd, q_rows, show_xlabel=False, annotate=True)
    draw_dot_panel(ax_c, c_rows, "(b) By primary cognitive load", show_xlabel=True)
    draw_delta_panel(ax_cd, c_rows, show_xlabel=True)

    fig.legend(
        handles=legend_handles(),
        loc="upper center",
        bbox_to_anchor=(0.46, 1.005),
        ncol=4,
        frameon=False,
        handletextpad=0.35,
        columnspacing=1.4,
    ...
    for ext in ("png", "pdf"):
        fig.savefig(OUT_DIR / f"{FIG_NAME}.{ext}", bbox_inches="tight", pad_inches=0.02)
    plt.close(fig)
